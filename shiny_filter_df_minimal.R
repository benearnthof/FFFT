...
skrrra <- function() {
  files_sources <- list.files(full.names = TRUE, path = "I:/airworthiness.p/01_Mitarbeiter/Benedikt Arnthof/wd/userLibrary/esquisse-master/R", pattern = "*.R")
  sapply(files_sources, source, .GlobalEnv)
}
ui <- fluidPage(
  tags$h2("Filtered data input"),
  
  
  # dataImportFileUI("dataset"),
  # radioButtons(
  #   inputId = "dataset",
  #   label = "Data:",
  #   # attach(dta) and stringinput as dta seems to work for some weird reason
  #   choices = c("dta",
